...
`readepi` allows importing data from several file types into R, including:

* all file formats in the [rio](https://cran.r-project.org/web/packages/rio/vignettes/rio.html) package.  * file formats that are not accounted for by `rio`    
...
When reading from REDCap, the function returns a list with 2 data frames, that represent respectively the data and its associated metadata.      

## Vignette
The vignette of the **readepi** contains detailed illustration about the used of the functions. This can be accessed by typing the command below:      
```{r eval=FALSE}
