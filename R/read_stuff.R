...
read_from_file <- function(file.path, sep = NULL, format = NULL, which = NULL, pattern = NULL) {
  ...
  if (file.exists(file.path) & !dir.exists(file.path)) {
    if (is.null(sep)) {
      if (!is.null(which) & !is.null(format)) {
        data <- rio::import(file.path, format = format, which = which)
      } else if (!is.null(which) & is.null(format)) {
        data <- rio::import(file.path, which = which)
      } else if (!is.null(format) & is.null(which)) {
        data <- rio::import(file.path, format = format)
      } else {
        data <- rio::import(file.path)
      }
  ...
  if (dir.exists(file.path) & length(list.files(file.path)) > 0) {
    if (!is.null(pattern)) {
      data <- rio::import_list(list.files(file.path, full.names = TRUE, pattern = pattern))
    } else {
  ...
  if (!is.null(credentials.file)) {
    ...
    if (credentials$dbms %in% c("redcap", "REDCap")) {
      res <- read_from_redcap(
        uri = credentials$host, token = credentials$pwd, project.id = credentials$project, id.position = id.position,
