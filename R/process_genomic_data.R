...
genome_assembly <- function(input.dir, assembly.dir, consensus.fasta.dir, num.samples, device = "MinION", partition = NULL) {
  ...
  if (device == "MinION") {
    ...
    medaka.model <- "r941_prom_hac_g303"
  }
  ...
  if (grepl("Darwin", system(sprintf("uname -a"), intern = TRUE)) | is.null(partition)) {
    ...
    foreach::foreach(i = 1:length(sample.dirs)) %dopar% {
      ...
      system(sprintf(
        "zsh -i genome_assembly.sh %s %s %s %d %s",
        sample.id,
        sample.dirs[i],
        assembly.dir,
        num.threads,
    ...
    if (num.samples > 5) {
      jid <- system(sprintf(
        "sbatch -J \"%s\" --partition=%s --qos=%s --array=[1-%d]%%5 covid_assembly_artic.sh --input-dir %s --output-dir %s",
        "genome_assembly",
        partition,
        partition,
        num.samples,
      ...
      jid <- system(sprintf(
        "sbatch -J \"%s\" --partition=%s --qos=%s --array=[1-%d] covid_assembly_artic.sh --input-dir %s --output-dir %s",
        "genome_assembly",
        partition,
        partition,
        num.samples,
    ...
    system(sprintf(
      "sbatch -J \"%s\" --partition=%s --qos=%s --dependency=afterok:%d put_consensus_together.sh --input-dir %s --output-dir %s",
      "concatenate_fasta",
      partition,
      partition,
      jid,
...
#' @examples clade_assignment(merged.fasta = system.file("extdata", "consensus_genomes.fasta", package = "readepi"), output.dir = "inst/extdata/clade_assignment")
#' @export
