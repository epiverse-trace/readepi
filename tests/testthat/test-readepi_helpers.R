test_that("subset_fields works as expected", {
  res <- subset_fields(data.frame = iris,fields = "Sepal.Length,Sepal.Width",
                       table.name = "iris")
...
test_that("subset_fields works as expected", {
  res <- subset_fields(data.frame = iris,fields = c("Sepal.Length","Sepal.Width"),
                       table.name = "iris")
  expect_type(res, "list")
  expect_length(res, 2)
  expect_named(res, c("data", "not_found"))
...
test_that("subset_records works as expected", {
  res <- subset_records(data.frame = iris, records = "setosa, virginica",
...
test_that("subset_records works as expected", {
  res <- subset_records(data.frame = iris, records = c("setosa", "virginica"),
                        id.position = 5, table.name = "iris")
  expect_type(res, "list")
  expect_length(res, 2)
  expect_named(res, c("data", "not_found"))
...
test_that("subset_fields fails as expected", {
  expect_error(res <- subset_fields(data.frame = NULL,fields = "Sepal.Length,Sepal.Width",
                                    table.name = "iris"),
               regexp = cat("Assertion on',data.frame,'failed: Must provide the data frame to subset from."))

  expect_error(res <- subset_fields(data.frame = NULL,fields = c("Sepal.Length","Sepal.Width"),
                                    table.name = "iris"),
               regexp = cat("Assertion on',data.frame,'failed: Must provide the data frame to subset from."))


  expect_error(res <- subset_fields(data.frame = iris,fields = NULL,
                                    table.name = "iris"),
               regexp = cat("Assertion on',fields,'failed: Must provide a vector or a string with the field to subset."))

  expect_error(res <- subset_fields(data.frame = iris,fields = "Sepal.Length,Sepal.Width",
                                    table.name = NULL),
               regexp = cat("Assertion on',table.name,'failed: Must provide the table name."))

  expect_error(res <- subset_fields(data.frame = iris,fields = c("Sepal.Length","Sepal.Width"),
                                    table.name = NULL),
               regexp = cat("Assertion on',table.name,'failed: Must provide the table name."))
  ...
  expect_error(res <- subset_fields(data.frame = NULL,fields = NULL,
                                    table.name = NULL),
...
test_that("subset_records fails as expected", {
  expect_error(res <- subset_records(data.frame=NULL, records="setosa, virginica",
                                     id.position = 5, table.name="iris"),
               regexp = cat("Assertion on',data.frame,'failed: Must provide the data frame to subset from."))

  expect_error(res <- subset_records(data.frame=NULL, records=c("setosa", "virginica"),
                                     id.position = 5, table.name="iris"),
               regexp = cat("Assertion on',data.frame,'failed: Must provide the data frame to subset from."))

  expect_error(res <- subset_records(data.frame = iris, records = NULL,
                                    id.position = 5, table.name = "iris"),
               regexp = cat("Assertion on',records,'failed: Must provide a vector or a string with the field to subset."))

  expect_error(res <- subset_records(data.frame = iris,records="setosa, virginica",
                                     id.position = -1, table.name = NULL),
               regexp = cat("Assertion on',id.position,'failed: Negative column number not allowed."))

  expect_error(res <- subset_records(data.frame = iris,records=c("setosa", "virginica"),
                                     id.position = -1, table.name = NULL),
               regexp = cat("Assertion on',id.position,'failed: Negative column number not allowed."))
...
test_that("read_credentials works as expected", {
  res <- read_credentials(file.path = system.file("extdata", "test.ini", package = "readepi"),
...
test_that("read_credentials fails as expected", {
  expect_error(res <- read_credentials(file.path = NULL,
                                       project.id = "Pats__Covid_19_Cohort_1_Screening"),
               regexp = cat("Assertion on',file.path,'failed: Must provide a path to the credential file."))

  expect_error(res <- read_credentials(file.path = c(system.file("extdata", "test.ini", package = "readepi"),system.file("extdata", "test.ini", package = "readepi")),
                                       project.id = "Pats__Covid_19_Cohort_1_Screening"),
               regexp = cat("Assertion on',file.path,'failed: Impossible to read from multiple credential files."))

  expect_error(res <- read_credentials(file.path = system.file("extdata", "test.ini", package = "readepi"),
                                       project.id = NULL),
