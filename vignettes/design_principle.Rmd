...
* `authenticate()`: used to authenticate a user (check who the user is) and check whether the user is authorized to access the requested database or API. This function is fundamental for the success of data import.
...
The `authenticate()` function requires the following parameters to authenticate the user:
...
We will ensure that once an argument is provided for the authentication, it is easily retrieved from the connection object, and used in the 2 main functions, hence preventing from been supplied in another request.
