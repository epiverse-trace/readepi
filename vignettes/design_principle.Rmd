...
It can be used to read from both HIS and RDBMS as shown in the below figure.
    ...
    * When reading from RDBMS:
        1. the value for the `type` argument is expected to be one of: "MS SQL", "MySQL", "PostgreSQL", or "SQLite".
        2. When the query parameters are provided as a `list`, they will be used to form the appropriate SQL query internally. That resulting SQL query is the one that is executed by the internal function `read_rdbms()`.
...
* The `read_epidata()` function relies primarily on two packages:
...
* The `read_from_server()` function depends on the following packages:
...
These functions also require system dependencies for OS-X and Linux systems, detailed in the vignette.
