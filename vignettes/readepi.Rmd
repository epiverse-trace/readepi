...
fields <- c("date,sex,age,ccg_code", "id,date_of_onset,district",
            "Sepal.Length,Species")  # the first string in the field vector corresponds to the name of the columns of interest from the first table specified in the `table.names` argument and so on...  
data <- readepi(credentials.file, 
                project.id = "TEST_READEPI", 
                driver.name = "ODBC Driver 17 for SQL Server", 
                table.name = table.names, 
...
records <- c("e38000191,e38000247,e38000020,e38000130,e38000122,e38000223",
             "9375,881,4539", 
             "setosa")  #"note that first string in the records vector corresponds to the records of interest from the first table specified in the `table.name` argument and so on... when the id column is not the first column in a table, use the `id.position`"
data <- readepi(credentials.file, 
                project.id = "TEST_READEPI", 
                driver.name = "ODBC Driver 17 for SQL Server", 
                table.name = table.names, 
