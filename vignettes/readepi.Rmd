...
The current implementation of **{readepi}** provides functions for reading data from two common HIS: ([SORMAS](https://www.sormas.org/), [DHIS2](https://dhis2.org/), and RDBMS such as MS SQL, MySQL, PostgreSQL, and SQLite. Other utility functions for accessing relevant files and data are also included in this package.
...
```{r rdbms-login, eval = identical(Sys.getenv("NOT_CRAN"), "true")}
...
```{r display-tables, eval = identical(Sys.getenv("NOT_CRAN"), "true") }
# DISPLAY THE LIST OF TABLES FROM A DATABASE OF INTEREST
tables <- show_tables(login = rdbms_login)
head(tables)
```

```{r rdbms-query, eval = identical(Sys.getenv("NOT_CRAN"), "true")}
...
```{r rdbms-data, eval = identical(Sys.getenv("NOT_CRAN"), "true")}
...
```{r rdbms-filter, eval = identical(Sys.getenv("NOT_CRAN"), "true")}
...
The package includes a set of pre-configured author names and IDs that can be used to populate the query above. The table below lists these built-in server details. 

```{r pak-data, echo=FALSE, eval=TRUE}
...
```{r dispaly-data, echo=FALSE, eval=TRUE}
dat |>
  kableExtra::kbl() |>
...
The current version of **{readepi}** supports reading data from two common HIS: [DHIS2](https://dhis2.org/), and [SORMAS](https://www.sormas.org/).
...
```{r dhis2-login, eval = identical(Sys.getenv("NOT_CRAN"), "true")}
...
```{r org-units, eval = identical(Sys.getenv("NOT_CRAN"), "true")}
# GET THE LIST OF ALL ORGANISATION UNITS IN AN HIERARCHICAL ORDER
org_units <- get_organisation_units(login = dhis2_login)
```

```{r display-org-units, echo=FALSE, eval = identical(Sys.getenv("NOT_CRAN"), "true")}
org_units |>
  kableExtra::kbl() |>
  kableExtra::kable_paper("striped", font_size = 14, full_width = TRUE) |>
  kableExtra::scroll_box(height = "200px", width = "100%",
...
```{r dhis2-programs, eval = identical(Sys.getenv("NOT_CRAN"), "true")}
# GET THE LIST OF ALL PROGRAMS
programs <- get_programs(login = dhis2_login)
```

```{r display-programs, echo=FALSE, eval = identical(Sys.getenv("NOT_CRAN"), "true")}
programs |>
  kableExtra::kbl() |>
  kableExtra::kable_paper("striped", font_size = 14, full_width = TRUE) |>
  kableExtra::scroll_box(height = "200px", width = "100%",
...
```{r dish2-elements, eval = identical(Sys.getenv("NOT_CRAN"), "true")}
# GET THE LIST OF ALL DATA ELEMENTS
data_elements <- get_data_elements(login = dhis2_login)
```

```{r display-elements, echo=FALSE, eval = identical(Sys.getenv("NOT_CRAN"), "true")}
data_elements |>
  kableExtra::kbl() |>
  kableExtra::kable_paper("striped", font_size = 14, full_width = TRUE) |>
  kableExtra::scroll_box(height = "200px", width = "100%",
...
```{r dhis2-stages, eval = identical(Sys.getenv("NOT_CRAN"), "true")}
...
```{r display-stages, echo=FALSE, eval = identical(Sys.getenv("NOT_CRAN"), "true")}
program_stages |>
  kableExtra::kbl() |>
  kableExtra::kable_paper("striped", font_size = 14, full_width = TRUE) |>
  kableExtra::scroll_box(height = "200px", width = "100%",
...
```{r target-units, eval = identical(Sys.getenv("NOT_CRAN"), "true")}
...
```{r display-target-units, echo=FALSE, eval = identical(Sys.getenv("NOT_CRAN"), "true")}
target_org_units |>
  kableExtra::kbl() |>
  kableExtra::kable_paper("striped", font_size = 14, full_width = TRUE) |>
  kableExtra::scroll_box(height = "200px", width = "100%",
...
```{r org-codes, eval = identical(Sys.getenv("NOT_CRAN"), "true")}
...
```{r display-codes, echo=FALSE, eval = identical(Sys.getenv("NOT_CRAN"), "true")}
data[1:50, 1:15] |>
  kableExtra::kbl() |>
  kableExtra::kable_paper("striped", font_size = 14, full_width = TRUE) |>
...
The [Surveillance Outbreak Response Management and Analysis System (SORMAS)](https://www.sormas.org/) is an open source eHealth system that optimises the processes used in monitoring the spread of infectious diseases and responding to outbreak situations. The `read_sormas()` function can be used to import data from SORMAS through its API.
...
```{r sormas-login, eval = identical(Sys.getenv("NOT_CRAN"), "true")}
...
```{r sormas-dict, eval = identical(Sys.getenv("NOT_CRAN"), "true")}
...
```{r sormas-cases, eval = identical(Sys.getenv("NOT_CRAN"), "true")}
...
```{r display-cases, echo=FALSE, eval = identical(Sys.getenv("NOT_CRAN"), "true")}
covid_cases[, 1:15] |>
  kableExtra::kbl() |>
  kableExtra::kable_paper("striped", font_size = 14, full_width = TRUE) |>
  kableExtra::scroll_box(height = "200px", width = "100%",
