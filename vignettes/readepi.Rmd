...
The `install_odbc_driver()` function can be used to installed the chosen driver on Mac.     
...
If after running this command, you still have issues related to the MS SQL driver installation, use the links below for more details about how to install the **odbc** package in R and the odbc drivers:     
...
Note that this requires **Ubuntu 16.04 and above**.     
```{bash eval=FALSE}
driver=17
sudo su
...
sudo ACCEPT_EULA=Y apt-get install -y mssql-tools${driver}
echo 'export PATH="$PATH:/opt/mssql-tools${driver}/bin"' >> ~/.bashrc
source ~/.bashrc
