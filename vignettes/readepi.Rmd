...
The `readepi()` function can read data from a single file or from multiple files stored in the same directory. This function expects the following arguments:  

1. `data_source`: the path to the file or directory of interest.
2. `format`:      a character or vector of characters used to specify the file format(s) of interest.
3. `sep`:         (optional) a separator between the columns in the file.
4. `which`:       (optional) a character specifying which objects to import, e.g., name(s) of sheet(s) in an Excel file.    
5. `pattern`:     (optional) a character vector of characters used to determine the type of file to read from the specified directory.

**Note** that the argument `sep` useful for reading files in formats not supported by the [rio](https://cran.r-project.org/web/packages/rio/vignettes/rio.html#Data_Import) package. When not provided, the function will try to identify the separator between the columns in the provided file.    
Several examples of how to use the `readepi()` function to import data from files of different formats are shown below.  
...
The `readepi()` function can also read data from one or multiple sheet(s) in an MS Excel file. If the file contains multiple sheets, the user must supply the names of the sheets of interest, as shown in the examples below.     
...
data <- readepi(
  data_source = file,
  which       = c("Sheet2", "Sheet1")
)
```

## Importing data from files located in the same directory

`readepi()` can read data from multiple files in the same directory. When the directory contains files of different formats, the `pattern` argument must be used to define the file type of interest. See the following examples.    
